...
export async function buildOnlinePets({ source, output, repository, releaseTag }) {
  ...
  try {
    ...
    await Promise.all([mkdir(packagesDirectory, { recursive: true }), mkdir(join(pagesDirectory, "catalog", "v1"), { recursive: true })]);
    await Promise.all([
...
async function readPets(sourceDirectory) {
  ...
  for (const directoryName of directories) {
    ...
    if (!files.every((file) => allowedFiles.includes(file)) || !files.includes("pet.json") || !files.includes("spritesheet.webp")) {
    ...
    if (sprite.length === 0 || sprite.length > 25 * 1024 * 1024) throw new Error(`${directoryName}: spritesheet size is invalid`);
    const dimensions = readWebpDimensions(sprite);
    const detectedSpriteVersion =
      dimensions.width === 1536 && dimensions.height === 1872 ? 1
      : dimensions.width === 1536 && dimensions.height === 2288 ? 2
      : 0;
    if (detectedSpriteVersion === 0) {
      throw new Error(`${directoryName}: spritesheet must be a Codex v1 (1536×1872) or v2 (1536×2288) atlas`);
    }
    if (manifest.spriteVersionNumber !== undefined && manifest.spriteVersionNumber !== detectedSpriteVersion) {
    ...
    for (const key of ["version", "author", "assetLicense", "minAppVersion"]) {
      if (typeof pet[key] !== "string" || !pet[key].trim()) throw new Error(`${directoryName}: ${key} must be a non-empty string`);
    }
...
function validateManifest(directoryName, manifest) {
  if (!manifest || typeof manifest !== "object") throw new Error(`${directoryName}: pet.json must be an object`);
  ...
  for (const field of ["displayName", "description"]) {
    if (typeof manifest[field] !== "string" || !manifest[field].trim()) throw new Error(`${directoryName}: ${field} is required`);
  }
  if (manifest.spritesheetPath !== "spritesheet.webp") throw new Error(`${directoryName}: spritesheetPath must be spritesheet.webp`);
  if (manifest.spriteVersionNumber !== undefined && manifest.spriteVersionNumber !== 1 && manifest.spriteVersionNumber !== 2) {
...
async function packagePet(pet, packagePath, stagingDirectory) {
  ...
  if (result.status !== 0) throw new Error(`failed to package ${pet.id}: ${result.stderr || result.error?.message || "tar failed"}`);
}
...
function readWebpDimensions(buffer) {
  ...
  for (let offset = 12; offset + 8 <= buffer.length; ) {
    ...
    const data = offset + 8;
    if (type === "VP8X" && size >= 10) return { width: buffer.readUIntLE(data + 4, 3) + 1, height: buffer.readUIntLE(data + 7, 3) + 1 };
